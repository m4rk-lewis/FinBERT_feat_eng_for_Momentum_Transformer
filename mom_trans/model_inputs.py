...
# from examples.run_dmn_experiment import mjl_timeframe_mod

NEWSCAL_MODE = True

""" this is just a function that returns the experiment NAME to allow auto-modification of the model inputs 
and setup for different timeframe data based on the NAME of the experiment:
...
"""
# mjl_timeframe = mjl_timeframe_mod()
# print(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>MJL_NAME ",mjl_timeframe)
...
class ModelFeatures:
    ...
    """
    # test_name = NAME use this for modding the code dependant on test type / timeframe

    def __init__(
        self,
        df,
        total_time_steps,
        start_boundary=2012,    # TODO - this was originally 1990
        test_boundary=2016,     # TODO - this was originally 2020
        test_end=2021,
        changepoint_lbws=None,
        train_valid_sliding=False,
        # add_buffer_years_to_test=1,
        ...
        if NEWSCAL_MODE: # TODO Modification
            self._column_definition.append(
                #### TODO this variable was originally not used and blanked out - I will now feed calendar data here and set to categorical
                (f"month_of_year", DataTypes.CATEGORICAL, InputTypes.KNOWN_INPUT)
                # (f"month_of_year", DataTypes.REAL_CATEGORICAL, InputTypes.KNOWN_INPUT)
            )
            #### TODO this month_of_year variable was originally not used and blanked out, it now contains our news calendar data
    ...
    def _batch_data(self, data, sliding_window):
        ...
        """
        # this works but is a bit of a mess
        data = data.copy()
    ...
    def _batch_data_smaller_output(self, data, sliding_window, output_length):
        ...
        Returns:
          Batched Numpy array with shape=(?, self.time_steps, self.input_size)
        """
        # TO DO this works but is a bit of a mess
        data = data.copy()
        data["date"] = data.index.strftime("%Y-%m-%d")
    ...
    def input_params(self):
        ...
        # TO DO artefact of previous code - need to clean up
        locations = {
