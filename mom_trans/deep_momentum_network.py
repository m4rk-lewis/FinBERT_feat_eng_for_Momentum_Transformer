...
class SharpeLoss(tf.keras.losses.Loss): # TODO SharpeLoss
...
class SharpeValidationLoss(keras.callbacks.Callback): # TODO SharpeValidationLoss
    #  check if weights already exist and pass in best sharpe
    def __init__(
        self,
        inputs,
        returns,
...
class DeepMomentumNetworkModel(ABC):
    ...
    def get_positions(
        ...
        captured_returns = returns * positions # TODO captured_returns = returns * positions
...
class LstmDeepMomentumNetworkModel(DeepMomentumNetworkModel):
    ...
    def model_builder(self, hp):
        ...
        adam = keras.optimizers.Adam(learning_rate=learning_rate, clipnorm=max_gradient_norm) # 'lr' depreciated, use learning rate instead
