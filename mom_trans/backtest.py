        ...
        for bp in BACKTEST_AVERAGE_BASIS_POINTS:
            ...
            all_captured_returns = _captured_returns_from_all_windows(
                ...
                filter_identifiers=(
                    None
                    if asset_class == "ALL"
                    # else asset_class_tickers.loc[asset_class, asset_class_tickers.columns[0]].tolist()
                    else list(asset_class_tickers.loc[asset_class, asset_class_tickers.columns[0]]) # string value has no attribe 'tolist' so modified this - probably because only 1 ticker
            ),
...
def run_single_window(
    ...
    # raw_data["date"] = raw_data["date"] # MOD: try to defeat the input errur in
    ...
    # get rid of everything and reset -
    del best_model
    gc.collect()
