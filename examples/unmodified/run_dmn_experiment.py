...
def main(
    ...
    ## Finbert TFT modifications ###################################################################################
    ...
    elif experiment == "FinBERT-TFT-120":
        architecture = "TFT"
        lstm_time_steps = 120
        changepoint_lbws = [0, 1]  # we dont really use cpd, but we want the model to think our FinBert features are cpd
    elif experiment == "FinBERT-TFT-72":
        architecture = "TFT"
        lstm_time_steps = 72
        changepoint_lbws = [0, 1]  # we dont really use cpd, but we want the model to think our FinBert features are cpd
        ...
        lstm_time_steps = 252
        changepoint_lbws = [0, 1]  # we dont really use cpd, but we want the model to think our FinBert features are cpd
    elif experiment == "FinBERT-LSTM-120":
        architecture = "LSTM"
        lstm_time_steps = 120
        changepoint_lbws = [0, 1]  # we dont really use cpd, but we want the model to think our FinBert features are cpd
    elif experiment == "FinBERT-LSTM-72":
        architecture = "LSTM"
        lstm_time_steps = 72
        changepoint_lbws = [0, 1]  # we dont really use cpd, but we want the model to think our FinBert features are cpd
    ################################################################################################################
    ...
    def get_args():
        ...
        parser.add_argument(
            ...
            choices=[
                ...
                "LSTM-LONG-126-21",     # Mod to compare against finbert dataset
                "TFT",
                "TFT-CPD-126-21",
                "TFT-SHORT",
                "TFT-SHORT-CPD-21",
                "TFT-SHORT-CPD-63",
                "FinBERT-TFT-252",      # Mod to compare include the  finbert dataset
                "FinBERT-TFT-120",      # Mod to compare include the  finbert dataset
                "FinBERT-TFT-72",       # Mod to compare include the  finbert dataset
                "FinBERT-LSTM-252",     # Mod to compare include the  finbert dataset
                "FinBERT-LSTM-120",     # Mod to compare include the  finbert dataset
                "FinBERT-LSTM-72",      # Mod to compare include the  finbert dataset
            ],
